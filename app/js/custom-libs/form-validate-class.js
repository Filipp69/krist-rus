class formValidator {
  ...
  changeFormFieldsEvent(formSingle, field) {
    ...
    formElements.forEach((single) => {
        if (field === single) {
            ...
            // Убираем класс form-error с ближайшего inputholder
            const inputHolder = field.closest('.inputholder');
            if (inputHolder) {
                inputHolder.classList.remove('form-error');
            }

            // Очищаем backend ошибку, если есть
  ...
  _showError(field) {
    ...
    // Добавляем класс ближайшему inputholder
    const inputHolder = field.closest('.inputholder');
    if (inputHolder) {
        inputHolder.classList.add('form-error');
    }
    ...
    if (this.needErrorMessage) {
        ...
        // Берем кастомное сообщение или сообщение по умолчанию
        if (textMessageType && this.customError && this.customError[textMessageType]) {
            textMessage = this.customError[textMessageType];
        } else {
            textMessage = this.defaultErrorMessage || 'Поле обязательно для заполнения';
        }

        // Проверяем, есть ли уже текст ошибки
        let getErrorTip = field.parentNode.querySelector(
            "[data-error-text]:not([data-error-text='backend'])"
        );

        if (!getErrorTip) {
            const errorTip = `<span class="error-text" data-error-text="">${textMessage}</span>`;
